...
# Path to local PT sessions ports file (same cfg format)
_local_ports_file = _pathlib.Path(__file__).parent / "ports"

def _read_local_sessions():
    """Read local PT sessions from the ports file.
    Returns list of dicts with keys: project, type, corner, model, process, machine, port, raw_line
    Same cfg format: #project,type,corner,model,process,machine,port
    """
    if not _local_ports_file.exists():
        return []
    sessions = []
    try:
        for line in _local_ports_file.read_text().splitlines():
            line = line.strip()
...
def report_timing():
    ...
    if ((corner == "")):
        ...
        return
    cmd = PT_CLIENT_PL + " -m " + tag + " -c \"report_timing "
...
def all_corner_slack():
    ...
    matching_lines = [line for line in open(PT_SERVER_CFG).readlines() if pattern in line]
    corner_list = []
    ...
    for cor in corner_list:
        cmd = "echo "+ cor + " `" + PT_CLIENT_PL + " -m " + selected_model.get() + "_" +selected_project.get() + "_" +selected_type.get() + "_" + cor + " -c \"get_attribute [get_timing_path  "
        if "min_" in cor:
            cmd = cmd + " -delay_type min "
        if (from_txt.get() != ""):
            cmd = cmd + "\ \-from\ " + from_txt.get()
...
def min_max_window():
    ...
    if ("max_" in corner) or (corner == ""):
        ...
        return
    cmd = PT_CLIENT_PL + " -m " + tag  + " -c \"get_attribute [get_attribute [get_timing_paths "
    if (from_txt.get() != ""):
        cmd = cmd + " -from " + from_txt.get()
    if (through_txt.get() != ""):
        cmd = cmd + " -through " + through_txt.get()
...
def analyze_cell_vt():
    """Open a popup with two inputs (cell, ref) and run ts_analyze_cell_vt."""
    global viewer
    if tag == "":
        messagebox.showerror('Python Error', 'Please select a corner first')
        return
    popup = Tk()
    popup.title("ts_analyze_cell_vt")
    Label(popup, text='Cell name:').pack()
    cell_entry = Entry(popup, width=70)
    cell_entry.pack()
    Label(popup, text='Ref name:').pack()
    ref_entry = Entry(popup, width=70)
    ref_entry.pack()
    def do_analyze():
        cell = cell_entry.get().strip()
        ref  = ref_entry.get().strip()
        if not cell or not ref:
            messagebox.showerror('Python Error', 'Please fill in both cell and ref')
            return
        cmd = PT_CLIENT_PL + " -m " + tag + " -c \"ts_analyze_cell_vt " + cell + " " + ref
        cmd += " \" | /usr/bin/less > /tmp/results_" + USER + " ; xterm -fn fixed -ls -sb -geometry 295x40+100+80 -T \"ts_analyze_cell_vt\" -e " + viewer + " -S /tmp/results_" + USER + " & "
        popup.destroy()
        os.system(cmd)
    Button(popup, text='Ok', command=do_analyze).pack()
...
def on_select(event):
    ...
    # Refresh local PT sessions dropdown from ports file
    _refresh_local_sessions()
    print(tag)
...
def _refresh_local_sessions():
    """Read ports file and populate the local sessions dropdown."""
    sessions = _read_local_sessions()
    labels = []
    for s in sessions:
        label = s['corner'] + "  (" + s['machine'] + ":" + s['port'] + ")"
        labels.append(label)
    local_session_cb['values'] = labels
    if labels and local_session_var.get() not in labels:
        local_session_var.set(labels[0])
    if not labels:
        local_session_var.set("")
        local_status_lbl.config(text="No sessions", fg="gray")

def _on_local_session_select(event=None):
    """When a local session is selected, set all the global variables so
    report_timing/draw_path/etc. work without additional clicks."""
    global tag
    sessions = _read_local_sessions()
    idx = local_session_cb.current()
    if idx < 0 or idx >= len(sessions):
        return
    s = sessions[idx]
    selected_project.set(s['project'])
    selected_type.set(s['type'])
    selected_corner.set(s['corner'])
    selected_model.set(s['model'])
    tag = s['model'] + "_" + s['project'] + "_" + s['type'] + "_" + s['corner']
    model_label.config(text='\n'.join(textwrap.wrap("Local Session: " + s['machine'] + ":" + s['port'], 64)))
    local_status_lbl.config(text="● selected", fg="blue")

def _ping_local():
    """Ping the selected local PT session."""
    sessions = _read_local_sessions()
    idx = local_session_cb.current()
    if idx < 0 or idx >= len(sessions):
        local_status_lbl.config(text="● not connected", fg="gray")
        return
    s = sessions[idx]
    host = s['machine']
    port = s['port']
    try:
        cmd = "echo 'ping' | netcat -w 3 " + host + " " + port + " && echo 'Online' > /dev/null || exit 1"
        ret = os.system(cmd)
        if not ret:
            local_status_lbl.config(text="● Online", bg='#0f0', fg='#fff')
        else:
            local_status_lbl.config(text="● Offline", bg='#f00', fg='#fff')
    except Exception:
        local_status_lbl.config(text="● Offline", bg='#f00', fg='#fff')

def _stop_local():
    """Send pt_server::pt_server_stop to the selected local PT session."""
    sessions = _read_local_sessions()
    idx = local_session_cb.current()
    if idx < 0 or idx >= len(sessions):
        local_status_lbl.config(text="No session selected", fg="gray")
        return
    s = sessions[idx]
    host = s['machine']
    port = s['port']
    label = s['corner']
    if not messagebox.askyesno("Stop Local Session",
                               "Stop local PT session  " + label + "  on " + host + ":" + port + "?"):
        return
    try:
        import subprocess
        result = subprocess.run(
            ["bash", "-c", "echo 'pt_server_stop' | netcat -w 5 " + host + " " + port],
            capture_output=True, text=True, timeout=10)
        local_status_lbl.config(text="● Stopped", fg="red")
        _refresh_local_sessions()
    except Exception as e:
        local_status_lbl.config(text="● Stop failed", fg="red")
        messagebox.showerror("Stop Error", str(e))

viewer = "/usr/bin/less"    
USER = os.environ['USER']

# PT server root directory — change this to use local copy
PT_SERVER_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cth2_ptserver_root")
PT_CLIENT_PL   = os.path.join(PT_SERVER_ROOT, "pt_client.pl")
PT_SERVER_CFG  = os.path.join(PT_SERVER_ROOT, "pt_server_c2dgbcptserver_cron.cfg")
root = tk.Tk()
...
projects = "gfcn2clienta0 pnc78icorea0 pnc78clienta0 pnc78clientb0 pnc78servera0 cgc78clienta0 lnc78a0 lnc78b0 lncn3clientb0 lncn3lnlcliena0 lncn3lnlclienb0 "
...
# ── Local PT Sessions frame ──────────────────────────────────────────────────
i = i + 1
f_local = tk.Frame(root, highlightbackground="green", highlightthickness=2)
f_local.grid(row=i, column=0, sticky="ew", columnspan=total_col, padx=4, pady=4)

tk.Label(f_local, text="Local PT Sessions:", font="Arial 12 bold").grid(
    row=0, column=0, padx=6, pady=4, sticky="w")

local_session_var = tk.StringVar(value="")
local_session_cb = Combobox(f_local, textvariable=local_session_var, width=55, state="readonly")
local_session_cb.grid(row=0, column=1, padx=4)
local_session_cb.bind('<<ComboboxSelected>>', _on_local_session_select)

tk.Button(f_local, text="↺", font="Arial 11",
          command=_refresh_local_sessions).grid(row=0, column=2, padx=2)

local_status_lbl = tk.Label(f_local, text="● not connected", font="Arial 11", fg="gray")
local_status_lbl.grid(row=0, column=3, padx=6)

tk.Button(f_local, text="Ping", font="Arial 11",
          command=_ping_local).grid(row=0, column=4, padx=4)

tk.Button(f_local, text="Stop", font="Arial 11", fg="red",
          command=_stop_local).grid(row=0, column=5, padx=4)
...
button5 = tk.Button(f2, text='ts_analyze_cell_vt', width=18, command=analyze_cell_vt)
button5.grid(row=13, column=0,columnspan = 2)

i = i + 1
f3 = tk.Frame(root, highlightbackground="blue", highlightthickness=2)
f3.grid(row=i, column=0, sticky="w")
...
f4.grid(row=i, column=1, sticky="w")
