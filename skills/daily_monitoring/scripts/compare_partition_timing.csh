...
set mailing_list = "\
par_exe     $USER hmuhsen gilkeren gfc_par_exe@intel.com     hmuhsen mandrea \n\
par_fe      $USER hmuhsen gilkeren gfc_par_fe@intel.com      lstambul ahaimovi navits \n\
par_fmav1   $USER hmuhsen gilkeren gfc_par_fmav1@intel.com   \n\
par_fmav0   $USER hmuhsen gilkeren gfc_par_fmav0@intel.com   \n\
par_meu     $USER hmuhsen gilkeren gfc_par_meu@intel.com     lstambul fnaseral mandrea \n\
par_mlc     $USER hmuhsen gilkeren gfc_par_mlc@intel.com     amenashe lstambul tsabek \n\
par_msid    $USER hmuhsen gilkeren gfc_par_msid@intel.com    lstambul mburak navits \n\
par_ooo_int $USER hmuhsen gilkeren gfc_par_ooo_int@intel.com rebelini gilchen damburg \n\
par_ooo_vec $USER hmuhsen gilkeren gfc_par_ooo_vec@intel.com hjarhi eabuyuni damburg ayarokh \n\
par_pm      $USER hmuhsen gilkeren gfc_par_pm@intel.com      fnaseral lstambul navits \n\
par_pmh     $USER hmuhsen gilkeren gfc_par_pmh@intel.com     fnaseral mandrea aholtzma \n\
par_vpmm    $USER hmuhsen gilkeren ayarokh \n\
par_tmul_stub $USER hmuhsen gilkeren \n\
"


set par = $1 
set test_tag = ${PROJECT_STEPPING}LATEST
set ref_tag = GOLDEN
...
ls -ltr $PROJ_ARCHIVE/arc/$ci_par/sta_primetime/ | grep -w ${PROJECT_STEPPING}LATEST | egrep -v "GOLDEN|arc_trans|0LATEST|4POWERROLLUP|PNC_DROP_" |tail -1 |awk '{print $(NF-2)}'      
ls -ltr $PROJ_ARCHIVE/arc/$ci_par/sta_primetime/ | grep -w GOLDEN | egrep -v "GOLDEN|arc_trans|0LATEST|4POWERROLLUP|PNC_DROP_" |tail -1 |awk '{print $(NF-2)}'
...
set ref_tag = `ls -ltr $PROJ_ARCHIVE/arc/$ci_par/sta_primetime/ | grep -w $ref_tag_version | egrep -v "GOLDEN|arc_trans|0LATEST|4POWERROLLUP|PNC_DROP_" | tail -1 | awk '{print $(NF-2)}'`
 if ($ref_tag == "") then
...
foreach scenario ($scenarios) 
    ...
    zcat $PROJ_ARCHIVE/arc/${ci_par}/sta_primetime/${PROJECT_STEPPING}LATEST/${ci_par}.$scenario.report_global_timing.rpt.gz | grep -e "Total" -e "WNS" -e "TNS" -e "NUM" | awk '{print "TST "$0}' > test_model
    zcat $PROJ_ARCHIVE/arc/${ci_par}/sta_primetime/GOLDEN/${ci_par}.$scenario.report_global_timing.rpt.gz | grep -e "Total" -e "WNS" -e "TNS" -e "NUM" | awk '{print "REF "$0}' > ref_model
...
if (`echo $mailing_list | grep "$ci_par " | wc -l ` == 1 ) then 
    set users_to_mail = `echo $mailing_list | grep "^ *$ci_par " |  awk '{for (i=2; i<=NF;i++) printf $i" "}'`
...
cat mail_to_send | sendmail $users_to_mail
