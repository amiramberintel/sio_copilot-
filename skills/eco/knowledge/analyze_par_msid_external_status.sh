...
# Corner: func.max_nom.T_85.typical
...
PARTITION="par_msid"
CORNER="func.max_nom.T_85.typical"
OUTDIR="/nfs/site/disks/sunger_wa/fc_data"

echo "==============================================================================="
echo "PAR_MSID EXTERNAL INTERFACE TIMING ANALYSIS - MAX_NOM CORNER (DAILY MODEL)"
echo "==============================================================================="
...
EXTERNAL_FILE="${OUTDIR}/par_msid_max_nom_external_paths.xml"
...
grep "blocks_impacted.*par_msid" "$EXTERNAL_FILE" > "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml"
PAR_MSID_PATHS=$(wc -l < "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml")
echo "Paths crossing par_msid boundary: $PAR_MSID_PATHS"

# ===== SECTION 3: Extract boundary pins =====
echo ""
echo "Extracting boundary pins..."
grep -o 'boundary_pins="[^"]*"' "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml" | \
    sed 's/boundary_pins="//;s/"$//' | \
    tr ',' '\n' | sed 's/[{}]//g' | sed 's/^ *//;s/ *$//' | sort -u > "${OUTDIR}/par_msid_max_nom_boundary_pins.txt"
UNIQUE_PINS=$(wc -l < "${OUTDIR}/par_msid_max_nom_boundary_pins.txt")
echo "Unique boundary pins: $UNIQUE_PINS"
echo "Saved to: ${OUTDIR}/par_msid_max_nom_boundary_pins.txt"
...
awk -F'slack="' '{if (NF>1) {split($2,a,"\""); print a[1]}}' "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml" | \
    awk '{
        slack = $1
...
}' "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml" | sort -t'|' -k2 -rn
...
}' "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml"
...
}' "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml" | sort -n | head -50 > "${OUTDIR}/par_msid_max_nom_top50_worst.txt"

echo "Rank  Slack(ps)  POR_Slack  Path_Group       Startpoint -> Endpoint"
echo "---------------------------------------------------------------------------------"
awk -F'\t' '{
    printf "%4d  %8s  %9s  %-15s  %s -> %s\n", NR, $1, $6, $2, $3, $4
}' "${OUTDIR}/par_msid_max_nom_top50_worst.txt"

echo ""
echo "Full details saved to: ${OUTDIR}/par_msid_max_nom_top50_worst.txt"
...
END {
    ...
    sort -n > "${OUTDIR}/par_msid_max_nom_pin_slack_summary.txt"

echo "Top 100 worst par_msid interface pins (by worst slack):"
echo ""
echo "Rank  Worst_Slack(ps)  Path_Count  Pin_Name"
echo "-------------------------------------------------------------------------------"
head -100 "${OUTDIR}/par_msid_max_nom_pin_slack_summary.txt" | \
    awk -F'\t' '{printf "%4d  %15s  %10s  %s\n", NR, $1, $2, $3}'

echo ""
echo "Full pin summary saved to: ${OUTDIR}/par_msid_max_nom_pin_slack_summary.txt"
...
}' "${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml" | sort -k3 -n
...
echo "==============================================================================="
echo "1. All external paths:            ${OUTDIR}/par_msid_max_nom_external_paths.xml"
echo "2. Par_msid crossing paths:       ${OUTDIR}/par_msid_max_nom_crossing_par_msid.xml"
echo "3. Boundary pins list:            ${OUTDIR}/par_msid_max_nom_boundary_pins.txt"
echo "4. Top 50 worst paths:            ${OUTDIR}/par_msid_max_nom_top50_worst.txt"
echo "5. Pin slack summary:             ${OUTDIR}/par_msid_max_nom_pin_slack_summary.txt"
echo "==============================================================================="
echo ""
