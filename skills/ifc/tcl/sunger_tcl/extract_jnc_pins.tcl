# Extract JNC pin locations from nlib
# Run with: fc_shell -f extract_jnc_pins.tcl

set nlib_path "/nfs/site/disks/kknopp_wa/JNC/TIP/core_server.nlib"
set outfile "/nfs/site/disks/sunger_wa/fc_data/my_learns/tp_file_to_JNC/jnc_pin_locations.csv"

puts "Opening nlib: $nlib_path"
open_lib $nlib_path -read

# Open the icore block (top-level)
set designs [get_designs -quiet *]
puts "Available designs: $designs"

# Try to open icore design
...
foreach_in_collection cell $parts {
    ...
    puts "Processing partition: $cell_name (bbox: $bbox)"
    
    # Get all pins on this partition
    set pins [get_pins -quiet -of_objects $cell]
    foreach_in_collection pin $pins {
        set pin_name [get_attribute $pin full_name]
        set pin_dir [get_attribute $pin pin_direction]
        ...
        # Extract just the signal name (remove partition prefix)
        regexp {^[^/]+/(.+)$} $pin_name -> sig_name
        if {![info exists sig_name]} { set sig_name $pin_name }
        
        puts $fd "$sig_name,$cell_name,$pin_name,$pct_x,$pct_y,[format %.5f $x],[format %.5f $y]"
        incr count
    }
}

close $fd
puts "\nWrote $count pin locations to $outfile"
